...
% ============================================================
% FEEDBACK TRACKS
%  inner (orange, short dash): re-tune -> warmup, run longer -> collection
%  outer (blue,  long  dash):  change sampler -> preparatory
% ============================================================
\coordinate (tunebusbottom)    at ([yshift=-1.5mm]a1.east -| 15.15,0);
\coordinate (tunebranch)       at (15.15,9.05);    % branch point: run longer (row of the collection boxes)
\coordinate (tunebustop)       at (15.15,4.95);    % top: re-tune (row of the tuning boxes)
...
% from warmup diagnostics (change sampler only); arrowhead marks the direction
\draw[fbsamplerarrow] (w3.east) -- (w3.east -| samplerbusbottom);

% inner track
\draw[fbtune] (tunebusbottom) -- (tunebustop);
\draw[fbtunearrow] (15.15,10.10) -- (15.15,9.50);
\draw[fbtunearrow] (15.15,7.90)  -- (15.15,7.30);
% branch into the collection phase: run longer
\draw[fbtunearrow] (tunebranch) -- (13.40,9.05);
\node[fbtunelabel, anchor=south, inner sep=1pt] at (14.27,8.97) {run longer};
% top into the warmup phase: re-tune
\draw[fbtunearrow] (tunebustop) -- (13.40,4.95);
\node[fbtunelabel, anchor=south, inner sep=1pt] at (14.27,4.87) {re-tune};
