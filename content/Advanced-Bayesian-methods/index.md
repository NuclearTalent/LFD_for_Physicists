...
* {ref}`demo:dealing-with-outliers` explores several ways to extend linear regression to treat outlier data.
* {ref}`sec:BayesLinear` describes a powerful technique that starts with the statistical model. 
* Multi-model inference is a large topic with many facets. One approach to dealing with multiple models for the same data is {ref}`sec:ModelSelection`, which gives a probabilistic ranking among candidate models. Another set of approaches, {ref}`sec:ModelMixing`, combine ("mix") results from the models rather than seeking the best, with the goal of more informed inference.
