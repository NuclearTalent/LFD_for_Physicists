...
See the {ref}`sec:VisualizingMCMC` created by Chi Feng for an illustration of this. In these simulations, the marginalization is performed by projecting the joint samples into a histogram on the $x$ or $y$ axis.
```
...
Such questions can be rephrased as: Given the joint PDF $\pdf{x,y}{I}$, what is $\pdf{z}{I}$, where $z=f(x,y)$? 
In this situation we can use marginalization and the product rule. 

\begin{equation}
\pdf{z}{I} = \int \pdf{z,x,y}{I}\,dx\,dy = \int \pdf{z}{x,y,I}\, \pdf{x,y}{I}\,dx\,dy.
\end{equation}
...
In addition, an approximate propagation of errors will be demonstrated in {numref}`sec:ErrorPropagationIII` in {prf:ref}`example:BayesianAdvantage:inferring-galactic-distances-revisited`.
...
$$
v_\mathrm{measured} = H d + \delta\mathcal{D},
$$
...
The results for the different analysis strategies (1-3 in the list above) should be obtained in {numref}`exercise:BayesianAdvantages:inferring-galactic-distances-ex`. A comparison figure that also includes the approximate error propagation from {prf:ref}`example:BayesianAdvantage:inferring-galactic-distances-revisited` is shown in the solution to this exercise.
````
...
\begin{align*}
\pdf{z}{I} &= \int \pdf{z}{x,y,I}\, \pdf{x,y}{I}\, dx\,dy = \int \delta(z-\sqrt{2}s) p(s|I) p(t|I)\, ds\,dt
\\
&= \left\{ 
\begin{array}{l}
...
1. A fixed value for $H$ can be assigned with the PDF $\pdf{H}{I} = \delta(H-H_0)$, where $\delta(x)$ is the Dirac delta function. We note that integrals over a delta function are given by $\int_{-\infty}^{+\infty} f(x) \delta(x-x_0)\, dx = f(x_0)$ such that
