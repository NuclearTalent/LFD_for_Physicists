...
* **Fast**: Uses [PyTensor](https://pytensor.readthedocs.io/en/latest/index.html) as its computational backend to compile through C, Numba or JAX, [run your models on the GPU](https://www.pymc-labs.io/blog-posts/pymc-stan-benchmark/), and benefit from complex graph-optimizations.
* **Batteries included**: Includes probability distributions, Gaussian processes, ABC, SMC and much more. It integrates nicely with [ArviZ](https://python.arviz.org/en/stable/index.html) for visualizations and diagnostics, as well as [Bambi](https://bambinos.github.io/bambi/) for high-level mixed-effect models.
