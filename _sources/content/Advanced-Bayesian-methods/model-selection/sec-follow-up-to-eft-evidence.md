...
where the Bayes' theorem denominators cancel. We'll take the ratio $p(M_    2|I)/p(M_1,I)$ to be one.
Thus we have:
...
**i) values of $a'$ that contribute to the integrand in the numerator of {eq}`eq:ratio_EFT` are determined by the likelihood peaked region.**
E.g., recall

```{image} ../assets/model_selection_lec_15_handdrawn.png
...
```{image} ../assets/model_selection1a_lec_15_handdrawn.png
:alt: Model selection figure
:class: bg-primary
:width: 200px
:align: right
...
How much parameter space collapses in the face of data? We thought initially that $a'$ could be anywhere in $\Delta a'$, but find after seeing the data that it is only in $\delta a'$. What a waste (less predictive) if $\delta a' \ll \Delta a'$!
...
```{image} ../assets/model_selection2_lec_15_handdrawn.png
:alt: Model selection figure
:class: bg-primary
:width: 200px
:align: center
...
```{image} ../assets/model_selection3_lec_15_handdrawn.png
:alt: Model selection figure
:class: bg-primary
:width: 200px
:align: center
...
The integrals to calculate are Gaussian in multiple variables: $\avec = (a_0, \ldots, a_k)$ plus $\bar{a}$. We can write them with matrices:
...
**ii) Use conjugacy.**
See the "[conjugate prior](https://en.wikipedia.org/wiki/Conjugate_prior)" entry in Wikipedia for details. 
